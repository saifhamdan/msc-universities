# University of Kassel Proof of Competence Draft

This draft is filled from the current University of Jordan CIS course inventory and cross-checked against the official University of Jordan department course pages that were reachable from this environment.

Where the official JU course page or syllabus was recoverable, the topic summary below follows that source closely. Where JU's CS-side pages were not retrievable from this environment, the topic summary stays conservative and transcript-grounded rather than overstating specificity.

Replace bracketed placeholders only where needed, especially for the seminar topic, project topic, and bachelor-thesis title if your official records use a more specific title.

## Credit Note

- The transcript values below are format as `5 ECTS credits` per course.
- According to the University of Jordan bachelor conversion table, `3 UJ credit hours` correspond to a maximum of `5 ECTS credits`.

## Personal Information

- Last Name: Hamdan
- First Name: Saif Allah
- Previous University: University of Jordan
- E-Mail: saifhamdan14@gmail.com
- Previous Field of Study: Computer Information Systems
- Standard Period of Study: 8 Semesters

## Practical Computer Science

| Module                               | Credits | Content, main topics covered in the course                                                                                                                                              |
| ------------------------------------ | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- |
| Data Structures 1                    | 5 CP    | fundamental data structures, recursion, algorithmic problem solving, and basic data organization for program design                                                                     |
| Object Oriented Programming 1        | 5 CP    | core object-oriented programming concepts including classes, objects, encapsulation, inheritance, polymorphism, and program design                                                      |
| Object Oriented Programming 2        | 5 CP    | advanced object-oriented design and implementation, abstraction, modularity, reusable components, and larger-scale Java-style programming                                               |
| Web Applications Development 1       | 5 CP    | core web application development concepts, client-server structure, interactive web pages, and basic web programming                                                                    |     |
| Introduction to Database Systems     | 5 CP    | DBMS architecture and environment, relational database design, SQL, conceptual design methods, security and integrity constraints, and practical relational database implementation     |
| Advanced Databases                   | 5 CP    | advanced database design and implementation, database technologies, schema design, and more advanced database processing concepts                                                       |
| Distributed Databases                | 5 CP    | distributed database architecture, fragmentation, replication, distributed query and transaction processing, consistency, and reliability in distributed data management                |
| Introduction to Software Engineering | 5 CP    | software processes and agile methods, requirements collection, system modeling, UML, software lifecycle activities, maintenance, project management, and professional software practice |
| Advanced Software Engineering        | 5 CP    | advanced software engineering methods, software architecture and quality, development organization, and maintainable large-scale software design                                        |
| Systems Analysis and Design          | 5 CP    | systems development life cycle, requirements analysis, process and data modeling, system design methods, and structured development of information systems                              |

## Technical Computer Science

Note: the transcript does not show a separately titled digital logic or computer architecture module. The strongest systems-oriented modules are listed below. For `Operating Systems` and `Computer Networks 1`, the JU source page was not directly recoverable here, so the summaries remain conservative.

| Module                                 | Credits | Content, main topics covered in the course                                                                                                                                                                          |
| -------------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Fundamentals of Information Technology | 5 CP    | introductory IT foundations, hardware and software basics, cloud-computing and web-technology concepts, IT careers, and a general perspective on project management, with both theoretical and practical components |
| Operating Systems                      | 5 CP    | processes, threads, scheduling, synchronization, memory management, file systems, and operating-system services                                                                                                     |
| Computer Networks 1                    | 5 CP    | network architectures and protocols, layered communication models, routing and switching, transport mechanisms, and network services                                                                                |
| Information Security and Privacy       | 5 CP    | information protection, privacy, access control, basic security mechanisms, and security issues in information systems                                                                                              |

## Theoretical Computer Science

| Module                  | Credits | Content, main topics covered in the course                                                                                                   |
| ----------------------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| Theory of Algorithms    | 5 CP    | algorithm design and analysis, correctness, asymptotic efficiency, and complexity-based reasoning                                            |
| Theory of Computation   | 5 CP    | formal languages, automata, computability, decidability, and theoretical models of computation                                               |
| Data Structures 2       | 5 CP    | advanced data structures, tree and graph representations, searching and sorting methods, and analysis of algorithmic efficiency              |
| Artificial Intelligence | 5 CP    | artificial-intelligence foundations including search, knowledge representation, reasoning, intelligent behavior, and problem-solving methods |

## Mathematics

| Module                   | Credits | Content, main topics covered in the course                                                          |
| ------------------------ | ------- | --------------------------------------------------------------------------------------------------- |
| Discrete Mathematics     | 5 CP    | logic, sets, relations, functions, combinatorics, graphs, proof techniques                          |
| Calculus I               | 5 CP    | limits, derivatives, integrals, functions, continuous change, mathematical analysis basics          |
| Principles of Statistics | 5 CP    | descriptive statistics, probability basics, random variables, estimation, hypothesis-testing basics |

## Special Modules and Other Items

| Module                                           | Credits | Content, main topics covered in the course                                                                                                                                                                                |
| ------------------------------------------------ | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Machine Learning and Neural Network              | 5 CP    | machine-learning foundations, neural-network models, training and prediction, and applied intelligent-system techniques                                                                                                   |
| Digital Image Processing                         | 5 CP    | digital-image representation, enhancement, filtering, transformation, feature extraction, and practical image-processing applications                                                                                     |
| Human Computer Interaction                       | 5 CP    | design, implementation, and evaluation of interactive systems for human use; ergonomics; interaction-design activities; data gathering; prototyping; evaluation paradigms and techniques; and universal design principles |
| Computer Graphics                                | 5 CP    | graphical representation, rendering basics, 2D and 3D graphics concepts                                                                                                                                                   |
| Multimedia                                       | 5 CP    | multimedia systems, digital media handling, integration of text, image, audio, and video                                                                                                                                  |
| Geographical Information Systems                 | 5 CP    | spatial data, GIS concepts, visualization, geographic data processing                                                                                                                                                     |
| Information Systems Projects Management          | 5 CP    | planning, organizing, scheduling, coordinating, and controlling information-systems projects, including teamwork and delivery management                                                                                  |
| Information System Innovation and New Technology | 5 CP    | innovation in information systems, emerging technologies, evaluation of new technical solutions, and technology-driven change                                                                                             |
| Computer Assisted Learning                       | 5 CP    | computer-supported learning systems, interactive learning environments, and technology-enhanced educational applications                                                                                                  |
| Management Information Systems                   | 5 CP    | information systems in organizations, business-process support, managerial information use, and decision-support-oriented system concepts                                                                                 |
| Learning & Research Skills                       | 5 CP    | academic research methods, literature search, source evaluation, structured writing, and oral presentation                                                                                                                |
| Documentation and Ethics                         | 5 CP    | technical documentation, professional communication, and ethical responsibilities in computing and information systems                                                                                                    |

## Seminar

| Item    | Draft text                                                                                                                                                                                                                                                                                                                        |
| ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Seminar | MO3TAMAD - Certified Assessments Platform. Seminar-style presentation topic connected to the graduation project, covering the problem of trusted online certification, requirements analysis, system architecture, role-based access control, exam lifecycle, anti-cheat signals, certificate generation, and evaluation results. |

## Project

| Item    | Draft text                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Project | Project 2, 5 CP. MO3TAMAD - Certified Assessments Platform. Team-based full-stack graduation project for conducting tamper-resistant online certification exams and issuing verifiable digital certificates. The project included requirements analysis, system and database design, backend API development in Go/Fiber, frontend development in React/TypeScript, MySQL and Redis integration, role-based authorization, timed exam workflow, anti-cheat monitoring, certificate generation, testing, deployment with Docker, and technical documentation. |

## Bachelor Thesis

| Item            | Draft text                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Bachelor Thesis | MO3TAMAD - Certified Assessments Platform. B.Sc. graduation project / thesis at King Abdullah II School of Information Technology, University of Jordan, 2023. Supervisors: Dr. Bilal Abu Salih, Dr. Reem Alfayez, and Dr. Saleh Sharaeh. The written thesis/report documented the problem definition, requirements analysis, UML and system modeling, database and architecture design, implementation of a secure online assessment platform, certificate verification workflow, and evaluation of the system as a trusted certification solution for the Arab job market. |

## Short Copy-Paste Version

If you want a more compact version for the DOCX table, use the same module lists above and keep the bottom three rows in this form:

- Seminar: MO3TAMAD - Certified Assessments Platform; presentation on trusted online certification, system requirements, architecture, anti-cheat mechanisms, certificate generation, and evaluation results.
- Project: Project 2, 5 CP, MO3TAMAD - Certified Assessments Platform; team-based full-stack graduation project covering analysis, design, implementation, testing, deployment, and documentation.
- Bachelor Thesis: MO3TAMAD - Certified Assessments Platform; B.Sc. graduation project / thesis report on the design, implementation, and evaluation of a trusted online certification platform.
